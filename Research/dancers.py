...
def find_new_theta_with_pizza_slice(curr_dancer, room):
    ...
    angles = []
    for n, dancer in enumerate(room.dancers):
        dx = dancer.x_next[-1] - x
        if abs(dx) > dancer.room_width/2:
            dx = -np.sign(dx) * (dancer.room_width - abs(dx))
        dy = dancer.y_next[-1] - y
        if abs(dy) > dancer.room_height/2:
            dy = -np.sign(dy) * (dancer.room_height - abs(dy))
...
def make_step(dancer, ax, next_fact=1):
    if ax == 'x':
        # return (dancer.x + (dancer.step_size + np.random.uniform()-0.5) * np.cos(dancer.direction)) % dancer.room_width
        return (dancer.x[-1] + (dancer.step_size * next_fact) * np.cos(dancer.direction)) % dancer.room_width
    if ax == 'y':
        # return (dancer.y + (dancer.step_size + np.random.uniform()-0.5) * np.sin(dancer.direction)) % dancer.room_height
        return (dancer.y[-1] + (dancer.step_size * next_fact) * np.sin(dancer.direction)) % dancer.room_height
    ...
    def draw_room(self):
        ...
        if self.show_ids:
            for dancer in room.dancers:
                plt.text(dancer.x[-1] + 1, dancer.y[-1] - 1, str(dancer.id))
...
class Dancer:
    ...
    def update_speed(self, curr_room):
        ...
        direction = get_average_angle(direction, self.direction, alpha=0.3)
    ...
    def make_step(self):
        ...
        self.x_next.append(make_step(self, 'x', next_fact=2))
        self.y_next.append(make_step(self, 'y', next_fact=2))

    def out_of_bounds(self):
        out_of_bounds = False
        if (self.y[-1] > self.room_height) or (self.y[-1] < 0):
            out_of_bounds = True
        if (self.x[-1] > self.room_width) or (self.x[-1] < 0):
            out_of_bounds = True
...
if __name__ == '__main__':
    ...
    room = Room(n_dancers=150, width=300, height=200, num_iters=20, speed_noise=0)
    # room.draw_room()
    ...
    p.dump(room, open('old_room.p', "wb"))
    fig = plt.figure()
