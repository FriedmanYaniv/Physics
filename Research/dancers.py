...
def calc_new_theta(room, dancer):
    interaction_r = 1
    # nn_dancers = find_nn_dancers(dancer.location_x, dancer.location_y, room)
    nn_dancers = find_nn_dancers_with_blocks(dancer.location_x, dancer.location_y, room)
    return 0


def find_nn_dancers_with_blocks(x, y, room):
...
class Room:
    ...
    def __init__(self, n_dancers, width, height):
        self.width = width
        self.height = height
...
class Dancer:
    ...
    def update_speed(self):
        global room
        global noise
        new_theta = calc_new_theta(room, self)
        self.direction = new_theta


noise = 1
room = Room(n_dancers=50, width=100, height=100)
